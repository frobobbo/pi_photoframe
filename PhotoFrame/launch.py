...
#Connect to Google Photos and get a list of photos in selected Album
	...
	for x in range(gUserCount):
		...
		for photo in photos:
			gphotoCount+=1
			print "FileName: "+photo["filename"]+str(gphotoCount)
			print "MimeType: "+photo["mimeType"]+str(gphotoCount)
			photoName = os.path.splitext(photo["filename"])[0]
			if photo["mimeType"]=="image/jpeg":
				googleExtList[photoName] = "jpg"
			else:
				googleExtList[photoName] = "heic"
			print "photoName: "+photoName+str(gphotoCount)
			googlePhotoList[photoName] = photo["baseUrl"]+"=w1024"
			try:
				googleCaptionList[photoName] = photo['description']
			except KeyError:
				pass
		print "Number of Photos in google Album: " + str(len(photos))
########################################################################
#                                                                      #
...
def RemovePhotosfromAlbum():
	...
	print "Photos in local directory: "+str(len(currPhotoList))
...
def DownloadPhotosfromGoogle():
	...
	for pic in googlePhotoList:
		...
		if pic not in currPhotoList or pic in googleCaptionList:
			...
			DLCount+=1
			print "DownloadCount: "+str(DLCount)
			...
			if googleExtList.get(pic).lower() == "heic":
				...
				#now that the file is converted, update the ext
				print "now that the file is converted, update the ext"
				googleExtList[pic] = "jpg"
				fullPic = pic+"."+str(googleExtList.get(pic))
				print "Updated fullPic: "+fullPic
			...
			#Finally - check if background needs to be Blurred
			BlurPic = Image.open(os.path.join(configdir,"photos",fullPic))
			wBlurPic, hBlurPic = BlurPic.size
			print "BlurPic Width: " + str(wBlurPic)
			print "BlurPic Height: " + str(hBlurPic)
#			if hBlurPic > wBlurPic:
			#Pic is Portrait Mode
			print "Pic is Portrait - Blur Background"
			BlurPortrait.makeFullframe(os.path.join(configdir,"photos",fullPic))
...
if Config.get('Plugins','Instagram') == 'True':
	try:
		...
		print "Exception Obj: " + str(exc_obj)
		print "Exception Line Number: " + str(exc_tb.tb_lineno)
